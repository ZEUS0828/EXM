...
export const getEquipoPorId = async (req, res) => {
    try {
        ...
        if (result.length <= 0) return res.status(404).json({ id_eq: 0, message: "Equipo no encontrado" });
        res.json(result[0]);
    } catch (error) {
...
export const deleteEquipo = async (req, res) => {
    try {
        ...
        if (rows.affectedRows <= 0) return res.status(404).json({ message: "No se pudo eliminar el equipo" });
        res.sendStatus(202);
    } catch (error) {
        return res.status(500).json({ message: "Error del lado del servidor" });
    }
