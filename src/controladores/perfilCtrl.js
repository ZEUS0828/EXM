...
export const getPerfilPorId = async (req, res) => {
    try {
        ...
        if (result.length <= 0) return res.status(404).json({ per_id: 0, message: "Perfil no encontrado" });
        res.json(result[0]);
    } catch (error) {
...
export const deletePerfil = async (req, res) => {
    try {
        ...
        if (rows.affectedRows <= 0) return res.status(404).json({ message: "No se pudo eliminar el perfil" });
        res.sendStatus(202);
    } catch (error) {
        return res.status(500).json({ message: "Error del lado del servidor" });
    }
