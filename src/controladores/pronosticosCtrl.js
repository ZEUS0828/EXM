...
export const getPronosticoPorId = async (req, res) => {
    try {
        ...
        if (result.length <= 0) return res.status(404).json({ id_pron: 0, message: "Pronóstico no encontrado" });
        res.json(result[0]);
    } catch (error) {
...
export const deletePronostico = async (req, res) => {
    try {
        ...
        if (rows.affectedRows <= 0) return res.status(404).json({ message: "No se pudo eliminar el pronóstico" });
        res.sendStatus(202);
    } catch (error) {
        return res.status(500).json({ message: "Error del lado del servidor" });
    }
