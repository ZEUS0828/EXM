...
export const getUsuarioPorId = async (req, res) => {
    try {
        ...
        if (result.length <= 0) return res.status(404).json({ id_usr: 0, message: "Usuario no encontrado" });
        res.json(result[0]);
    } catch (error) {
...
export const postUsuario = async (req, res) => {
    try {
        ...
        const [rows] = await conmysql.query(
            `INSERT INTO usuario (cedula, nombres, direccion, telefono, fecha_registro, usuario, clave, per_id) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
export const putUsuario = async (req, res) => {
    try {
        ...
        const [result] = await conmysql.query(
            `UPDATE usuario SET cedula = ?, nombres = ?, direccion = ?, telefono = ?, 
             fecha_registro = ?, usuario = ?, clave = ?, per_id = ? WHERE id_usr = ?`,
...
export const deleteUsuario = async (req, res) => {
    try {
        ...
        if (rows.affectedRows <= 0) return res.status(404).json({ message: "No se pudo eliminar el usuario" });
        res.sendStatus(202);
    } catch (error) {
        return res.status(500).json({ message: "Error del lado del servidor" });
    }
